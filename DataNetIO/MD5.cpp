...
void CMD5::update(const CString &str){
	CString cstr(str);
	int len = (cstr.GetLength()+1)*sizeof(TCHAR);
	byte* bBuf = new byte[len];	
	memcpy(bBuf,cstr.GetBuffer(),len);
	cstr.ReleaseBuffer();
	update((const byte*)bBuf, len);
	delete bBuf;
}
...
CString CMD5::toCString()
{
	return bytesToHexCString(digest(), 16);
}
...
CString CMD5::bytesToHexCString(const byte *input, size_t length){
	CString str;
